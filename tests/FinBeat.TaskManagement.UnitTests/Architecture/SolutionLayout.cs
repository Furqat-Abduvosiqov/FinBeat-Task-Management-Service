...
// against the dependency graph the build *declares*.
//
...
//
// The project list comes from the solution file rather than from globbing the directory tree. That
// is the semantically correct source - the solution defines which projects are part of it - and it
// is also the only version that survives a second checkout of the same repository nested inside the
// working tree, which is exactly what a git worktree under the repository root produces. Globbing
// finds both copies and every project name collides.
internal static partial class SolutionLayout
{
    ...
    /// <summary>
    /// The project names <paramref name="projectName"/> declares a ProjectReference to, sorted and de-duplicated.
    /// </summary>
    ...
    private static FileInfo FindSolutionFile()
    {
        ...
        // checkout's own solution when it runs from inside a nested worktree.
        for (var directory = new DirectoryInfo(AppContext.BaseDirectory); directory is not null; directory = directory.Parent)
        {
            var solution = directory.EnumerateFiles("*.sln").FirstOrDefault();
    ...
    private static Dictionary<string, FileInfo> ReadProjectsFromSolution()
    {
        ...
        foreach (var match in SolutionProjectLine().Matches(File.ReadAllText(solution.FullName)).Cast<Match>())
        {
            var relativePath = match.Groups["path"].Value;

            // Solution folders are recorded by the same line shape but carry no project extension.
    ...
    // Project("{type guid}") = "Display name", "relative\path.csproj", "{project guid}"
